...
# Output Directory
model_dir = 'models/monai_unetr_lightning_crop192_dice/'
...
# train
trainer.fit(unetr_module, train_loader, val_loader)

eval_num = 1
plt.figure("train", (12, 6))
plt.subplot(1, 2, 1)
plt.title("Iteration Average Loss")
x = [eval_num * (i + 1) for i in range(len(unetr_module.epoch_loss_values))]
y = unetr_module.epoch_loss_values
plt.xlabel("Iteration")
plt.plot(x, y)
plt.subplot(1, 2, 2)
plt.title("Val Mean Dice")
x = [eval_num * (i + 1) for i in range(len(unetr_module.metric_values))]
y = unetr_module.metric_values
plt.xlabel("Iteration")
plt.plot(x, y)
...
case_num = 4
# unetr_module.load_from_checkpoint(os.path.join(model_dir, "unetr_model_epoch=0.ckpt"))
unetr_module.eval()
unetr_module.to(device)
